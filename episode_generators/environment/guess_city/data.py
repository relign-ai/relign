...
from common.types import Reward, Done, History, Text, TextHistory, Trajectory
...
def create_trajectory_from_history(
    ...
    return Trajectory(text_history, tuple(reward), done)
...
def create_trajectory_from_conversation(conversation: Conversation, max_conversation_len: int=20, recompute_done: bool=False) -> TextTrajectory:
    ...
    done_flag = Done(value=done)

    return Trajectory(history=History(history=tuple(text_history)), reward=reward, done=done_flag)
...
def create_dt_text_history_from_trajectory(text_trajectory: Trajectory, max_conversation_len: int=20) -> TextHistory:
