...
from relign.utils.notebook_utils import get_repo_dir
        ...
        try:
            ...
            for conn in connections:
                if conn.laddr.port == port:
                    logger.info(
                        f"Found process {proc.name()} with PID {proc.pid} using port {port}"
                    )
                    os.kill(proc.pid, 9)
                    return
        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess) as e:
            logger.debug(f"Skipping process due to exception: {e}")
        except ValueError as e:
            logger.debug(f"Skipping process due to ValueError: {e}")
        except Exception as e:
            logger.debug(f"Unexpected error occurred while checking process: {e}")
