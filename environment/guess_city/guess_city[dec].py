...
class BatchedGuessMyCityEnv:
    ...
    def generate_answers(self, questions):
        ...
        inputs = [
            f"Your home town is {curr_word}." + question
            for curr_word, question in zip(curr_words, questions)
        ]
        encoder_ids = self.tokenizer(inputs, padding=True, return_tensors="pt").to(
            self.model.device
        )
    ...
    def step(self, questions):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            jobs = [
                executor.submit(env._step, q, a)
                for env, q, a in zip(self.env_list, questions, answers)
            ]
            results = [job.result() for job in jobs]
        return results


EnvironmentFactory.register_environment("guess-city-dec", BatchedGuessMyCityEnv)
